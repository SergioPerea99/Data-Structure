...
public:
      ...
      typename list<T>::iterator inicio(){ return puntos.begin(); };
      typename list<T>::iterator fin(){ return puntos.end(); }; //DUDA: realmente este método no es necesario quizas?
...
public:
    ...
    //TODO: POR IMPLEMENTAR ESTOS 3 METODOS
...
MallaRegular<T>::MallaRegular(float aXMin, float aYMin, float aXMax, float aYMax, int nDivX, int nDivY):
    ...
    tamaCasillaX((int)((aXMax-aXMin)/nDivX)), tamaCasillaY((int)((aYMax-aYMin)/nDivY))
{
...
std::vector<T>& MallaRegular<T>::buscarRango(float rxmin, float rymin, float rxmax, float rymax){
    vector<T> devolver; //DUDA: este método devolverá todos los valores T que pertenezcan al rango vd?
    ...
    for(float i = xMin; i < xMax; i += tamaCasillaX){
        for (float j = yMin; j  < yMax; j += tamaCasillaY){
            ...
            for (typename list<T>::iterator it = c->inicio(); it != c->fin(); it++) {
                if ((*it).getX() >= rxmin && (*it).getX() <= rxmax)//Está en el intervalo del eje x
                    if ((*it).getY() >= rymin && (*it).getY() <= rymax)//Está en el intervalo del eje y  
